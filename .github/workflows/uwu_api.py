...
def sql_newflight():
	...
	try:
		...
		print(f"Error in sql_newflight: {str(e)}")
...
def sql_editflight():
	...
	try:
		...
		flight_data = {
			"flight_number": norm_flight_number(flight[0]),
			"departure": flight[1],
			"arrival": flight[2],
			"datetime": flight[3],
			"status": flight[4].capitalize(),
			"event_id": flight[5]
		}
...
def sql_schedule():
	...
	conn.close()
	if not flights:
		print("Flight schedule is empty")
...
@app.route('/sql/airstats/', methods=['POST'])
def sql_airstats():
	data = request.get_json()
	...
	try:
		...
		announcement = data.get("anns")
		flights = data.get("flight_channel")

		if not all([airline, guild, announcement, flights]):
			return jsonify("Missing required fields"), 400

		conn = sqlite3.connect('airstats.db')
		cursor = conn.cursor()

		if cursor.execute("SELECT * FROM airlines WHERE airline = ?", (airline,)).fetchone():
			cursor.execute("UPDATE airlines SET guild = ?, announcement = ?, flights = ? WHERE airline = ?",
						   (str(guild), str(announcement), str(flights), airline))
		else:
			cursor.execute("INSERT INTO airlines VALUES (?, ?, ?, ?)",
						   (airline, str(guild), str(announcement), str(flights)))

		conn.commit()
		conn.close()
		return jsonify(f"Airline {airline} updated"), 200
...
@app.route('/sql/airstats/get/', methods=['GET'])
def sql_airstats_get():
	try:
		conn = sqlite3.connect('airstats.db')
		cursor = conn.cursor()
		...
		for airline in airlines:
			airline_dict[airline[0]] = {"guild": airline[1], "announcement": airline[2], "flights": airline[3]}
